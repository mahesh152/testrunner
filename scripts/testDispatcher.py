...
class Dispatcher:
    ...
    def getNumberOfServers(self, iniFile):
        f = open(iniFile)
        contents = f.read()
        f.close()
        return contents.count('dynamic')

    def getNumberOfAddpoolServers(self, iniFile, addPoolId):
        f = open(iniFile)
        contents = f.read()
        f.close()
        try:
    ...
    def rreplace(self, str, pattern, num_replacements):
        return str.rsplit(pattern, num_replacements)[0]

    def getNumberOfAvailableServers(self, getAvailUrl):
        return httplib2.Http(timeout=60).request(getAvailUrl, 'GET')

    def getSlave(self, data):
        if 'slave' in data:
            return data['slave']
        else:
            return 'P0'

    def grabServers(self, descriptor, options, testsToLaunch):
        ...
        if options.serverType.lower() == 'docker':
            getServerURL = 'http://' + SERVER_MANAGER + \
                           '/getdockers/{0}?count={1}&os={2}&poolId={3}'. \
                               format(descriptor, testsToLaunch['serverCount'], \
                                      options.os, options.poolId)
        else:
            getServerURL = 'http://' + SERVER_MANAGER + \
                           '/getservers/{0}?count={1}&expiresin={2}&os={3}&poolId={4}'. \
                               format(descriptor, testsToLaunch['serverCount'], testsToLaunch['timeLimit'], \
                                      options.os, options.poolId)
        ...
        if options.serverType.lower() != 'docker':
            ...
            if len(json.loads(content)) != testsToLaunch['serverCount']:
                return None

        r2 = json.loads(content)
        servers = json.dumps(r2).replace(' ', '').replace('[', '', 1)
        return self.rreplace(servers, ']', 1)
                    ...
                    if 'implementedIn' not in data or releaseVersion >= float(data['implementedIn']):
                        if 'jenkins' in data:
                            ...
                            testsToLaunch.append({
                                ...
                                'iniFile':data['config'],
                                'serverCount':self.getNumberOfServers(data['config']),
    ...
    #     if options.framework.lower() == "jython":
        ...
        while len(testsToLaunch) > 0:
            try:
                ...
                response, content = self.getNumberOfAvailableServers(getAvailUrl)
                if response.status != 200:
                   ...
                   time.sleep(POLL_INTERVAL)
                    ...
                    if haveTestToLaunch:
                        ...
                        descriptor = urllib.quote(testsToLaunch[i]['component'] + '-' +
                                                  testsToLaunch[i]['subcomponent'] +'-' +
                                                  time.strftime('%b-%d-%X') + '-' + options.version )

                        servers = self.grabServers(descriptor, options, testsToLaunch[i])
                        if servers is None:
                            continue
                            ...
                            if options.serverType.lower() != 'docker':
                                ...
                                if testsToLaunch[i]['addPoolServerCount']:
                                    addPoolServers = content2.replace(' ','')\
                                                       .replace('[','', 1)
                                    addPoolServers = self.rreplace(addPoolServers, ']', 1)
